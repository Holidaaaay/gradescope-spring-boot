...
### 子任务
1. [ ] **创建 `AdminController`**，路径为 `/admin/**`。
2. [ ] **端点**: `GET /admin/users` — 分页用户列表（查询参数：`pageNum`、`pageSize`、`role`、`status`）。
3. [ ] **端点**: `GET /admin/users/{id}` — 管理员查看任意用户。
4. [ ] **端点**: `PATCH /admin/users/{id}/status` — 启用/禁用用户（`status: 0 或 1`）。
5. [ ] **端点**: `GET /admin/dashboard/stats` — 统计：总用户数、总课程数、总作业数、总提交数。
6. [ ] **创建 `AdminUserListVO`**（可包含比普通 `UserVO` 更多的字段）。
7. [ ] **创建 `PageResult<T>`** 分页响应包装器（为后续 PageHelper 做准备）。

### 验收标准
- [ ] `GET /admin/users` 返回分页列表；仅 ADMIN 角色可访问。
- [ ] `PATCH /admin/users/{id}/status` 正确切换数据库中的用户状态。
- [ ] 已禁用用户（`status = 0`）无法登录。
- [ ] 仪表盘统计数据反映真实数据库计数。

### 测试方法
1. 对 `/admin/**` 进行不同角色的 MockMvc 测试 → 断言非管理员返回 403。
2. 集成测试：禁用用户 → 尝试登录 → 断言失败。
3. 验证分页参数生效（limit、offset 等价物）。

### 代码审查清单
- [ ] 所有 `/admin/**` 端点已正确保护。
- [ ] 即使在管理员列表中也不直接暴露 `passwordHash`。
- [ ] 状态切换是原子的（使用 `@Transactional`）。
- [ ] 分页默认值合理（pageNum=1, pageSize=10）。

### 推送条件
- [ ] 管理员端点已用管理员与非管理员 JWT 测试。
- [ ] `mvn clean test` 通过。
