...
| 11 | 所有用户可见文案使用中文（标题、标签、占位符、按钮、提示、错误信息）。 | 代码审查 |

#### 9.4.3 前端设计系统

为保证后续所有页面（学生、教师、管理员）视觉风格统一，前端必须遵循以下设计系统。

**设计主题**: “学术桌面 / 批改纸（Academic Desk / Graded Paper）”。整体感觉克制、可信、略带编辑感，避免常见 SaaS 默认色板。

**色彩体系**

| CSS 变量 | 色值 | 用途 |
|---|---|---|
| `--color-desk` | `#e8e8e6` | 页面/表单区背景 |
| `--color-paper` | `#fdfdfb` | 卡片背景 |
| `--color-ink` | `#1c1c1c` | 主文本 |
| `--color-graphite` | `#6b6b6b` | 次级文本、占位符、图标 |
| `--color-rule` | `#b8323e` | 主品牌色（批改红）：按钮、链接、聚焦环、Margin Rule |
| `--color-rule-dark` | `#922a33` | 悬停/按下状态 |
| `--color-panel` | `#2d2d2d` | 品牌面板/深色背景 |
| `--color-panel-ink` | `#f4f4f2` | 深色背景上的主文本 |
| `--color-panel-muted` | `#a0a0a0` | 深色背景上的次级文本 |
| `--color-border` | `#e0e0de` | 输入框边框、分隔线 |
| `--color-border-focus` | `#b8323e` | 输入框聚焦边框 |

完整变量定义见 `gradescope-frontend/src/style.css`。

**字体**

| 用途 | 字体 | 回退 |
|---|---|---|
| 标题/品牌 | Source Serif 4 | Georgia, "Noto Serif SC", serif |
| UI/正文 | Sora | Inter, "PingFang SC", "Microsoft YaHei", sans-serif |
| 数据/代码 | JetBrains Mono | "Fira Code", ui-monospace, monospace |

通过 `index.html` 加载 Google Fonts；生产环境若无法访问 Google Fonts，应使用系统回退字体。

**标志性元素：Margin Rule（批改建）**

- 灵感来自试卷页边距的红色批注线。
- 在认证卡片左侧使用 2px 红色竖线 + 刻度标记；移动端缩为 1px。
- 仅作装饰，必须设置 `aria-hidden="true"`。
- 后续可在评分页、作业详情页等教育场景中以变体形式复用，保持视觉识别度。

**表单与组件**

- 统一使用 Element Plus，但通过 CSS 变量覆盖视觉风格：
  - 输入框圆角 `--radius-md`，聚焦时显示红色光环（`0 0 0 1px var(--color-border-focus) inset, 0 0 0 3px rgba(184, 50, 62, 0.1)`）。
  - 主按钮使用 `--color-rule` 背景，悬停时背景变为 `--color-rule-dark` 并上浮 2px。
  - 文字链接使用 `--color-rule`，悬停变 `--color-rule-dark`。
- 表单标签 13px、600 字重、顶部对齐。
- 必填项用红色星号 `*` 标识；选填项用分隔线分组。

**动效**

- 页面级入场动画仅用于整体卡片/布局，**禁止**给按钮、输入框等关键交互元素设置 `opacity: 0` + `animation forwards` 的入场动画，防止元素不可见。
- 微交互：输入框聚焦光环、按钮悬停上浮/按下、链接悬停下划线。
- 必须尊重 `prefers-reduced-motion: reduce`，非必要动画在该模式下关闭。

**响应式断点**

| 断点 | 行为 |
|---|---|
| ≥1024px | 桌面双栏，品牌面板完整显示 |
| 768–1023px | 品牌面板折叠为顶部横幅 |
| <768px | 品牌面板仅保留 Logo，表单卡片全宽 |
| <480px | 品牌面板隐藏，表单占满视口 |

**文案语言**

- 所有用户可见文案使用**中文**。
- 包括页面标题、表单标签、占位符、按钮、提示信息、验证错误、空状态、页脚链接。
- 后端返回的英文错误消息，前端应映射为中文提示（或在后端支持中文 message 后统一处理）。

**无障碍**

- 文本与背景对比度符合 WCAG 2.1 AA。
- 装饰性元素（Margin Rule、背景图形）使用 `aria-hidden="true"`。
- 聚焦状态可见，键盘 Tab 顺序符合视觉顺序。
...
| 2026-06-25 | 1.2 | **新增前端设计系统规范**: 在 9.4.3 节定义统一的色彩体系（批改红/纸张白/石墨黑）、字体（Source Serif 4 + Sora）、Margin Rule 标志性元素、表单/按钮样式、动效规范、响应式断点、中文文案要求和无障碍标准；在 9.4.2 前端强制规则中增加“所有用户可见文案使用中文”。 | AI 助手 |
